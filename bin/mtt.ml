...
let parse_from_e : type a. a ast_kind -> input_kind -> (a, error) Result.t =
  fun ast_kind source ->
    parse_from ast_kind source |> Result.map_error ~f:(fun parse_error ->
...
let parse_and_typecheck_string expr_str typ_str =
  ...
  Typechecker.check ast typ |> Result.map_error ~f:(fun eval_err ->
...
let parse_and_typeinfer source =
  ...
  Typechecker.infer ast |> Result.map_error ~f:(fun eval_err ->
...
let parse_and_eval source =
  ...
  let%bind ast = parse_from_e Term source in
  Evaluator.eval ast |> Result.map_error ~f:(fun eval_err ->
...
let parse =
  Command.basic
    ~summary:"Parsing and printing back"
    Command.Let_syntax.(
      let%map_open
        expr_opt = flag "-e" (optional string) ~doc:"string Parse and print back the given [expression]"
      and filename =
      ...
      fun () ->
        ...
        | None -> filename |> input_kind_of_filename |> parse_from_e Term |> print_parsing_result)
...
let check =
  Command.basic
    ...
    Command.Let_syntax.(
      let%map_open
        expr_opt = flag "-e" (optional string) ~doc:"string [expression] to typecheck"
      and
        type_opt = flag "-t" (optional string) ~doc:"string [type] to typecheck"
      in
      fun () ->
        ...
        | Some _, None ->
            Out_channel.output_string stderr "Error: please provide type using -t flag";
            Out_channel.newline stderr
        | None, Some _ ->
            Out_channel.output_string stderr "Error: please provide expression using -e flag";
            Out_channel.newline stderr
        | None, None ->
            Out_channel.output_string stderr "Error: please provide expression and type as CLI flags";
            Out_channel.newline stderr)
...
let infer =
  Command.basic
    ...
    Command.Let_syntax.(
      let%map_open
        expr_opt = flag "-e" (optional string) ~doc:"string Infer the type of the given [expression]"
      and filename =
        anon (maybe_with_default "-" ("filename" %: Filename.arg_type))
      in
      fun () ->
        match expr_opt with
        | Some expr -> parse_and_typeinfer (String expr) |> print_typeinfer_result
        | None -> filename |> input_kind_of_filename |> parse_and_typeinfer |> print_typeinfer_result)
...
let eval =
  Command.basic
    ...
    Command.Let_syntax.(
      let%map_open
        expr_opt = flag "-e" (optional string) ~doc:"string Evaluate the given expression"
      and filename =
        anon (maybe_with_default "-" ("filename" %: Filename.arg_type))
      in
      fun () ->
        ...
        | None -> filename |> input_kind_of_filename |> parse_and_eval |> print_eval_result)
