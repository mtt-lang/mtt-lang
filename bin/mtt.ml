...
let compile_expr source_file source_arg =
  let run_interepter bytecode =
    let open Result.Let_syntax in
    let%bind valuecam = CamInterpreter.interept [ Cam.VUnit ] bytecode in
    return @@ Cam.cam2val valuecam
  in
  match osource source_file source_arg with
  ...
  | Some source -> (
      ...
      | Ok insts -> (
          let bytecode = Cam.genidx insts [] in
          match run_interepter bytecode with
          | Ok value ->
              let document = PrettyPrinter.Doc.of_val value in
              PPrint.ToChannel.pretty 1.0 80 stdout document;
              Out_channel.newline stdout;
              `Ok ()
          | Error _ -> `Error (false, "Error in generated bytecode"))
      | Error err_msg -> `Error (false, err_msg))

let disassemble_expr source_file source_arg =
  match osource source_file source_arg with
  | None -> `Error (true, "Please provide exactly one expression to dissasemble")
  | Some source -> (
      match Util.parse_and_compile source with
      | Ok insts ->
          let bytecode = Cam.genidx insts [] in
          let document = PPrint.string @@ Cam.dump_bytecode bytecode in
          PPrint.ToChannel.pretty 1.0 80 stdout document;
          Out_channel.newline stdout;
          `Ok ()
      | Error err_msg -> `Error (false, err_msg))
...
let compile_cmd =
  let source_file =
    let doc = "The file with expression to compile." in
    Arg.(value & pos 0 (some non_dir_file) None & info [] ~docv:"FILE" ~doc)
  in
  let source_arg =
    let doc = "The expression to compile given as a CLI argument." in
    Arg.(
      value
      & opt (some string) None
      & info [ "e"; "expression" ] ~docv:"EXPRESSION" ~doc)
  ...
  let man =
    [
      `S Manpage.s_description;
      `P
        "Compiles and evaluates an expression either from stdin, a file, or \
         given as a command-line parameter";
      `Blocks help_secs;
    ]
  in
...
let dis_cmd =
  let source_file =
    let doc = "The file with expression to dissasemble." in
    Arg.(value & pos 0 (some non_dir_file) None & info [] ~docv:"FILE" ~doc)
  in
  let source_arg =
    let doc = "The expression to dissasemble given as a CLI argument." in
    Arg.(
      value
      & opt (some string) None
      & info [ "e"; "expression" ] ~docv:"EXPRESSION" ~doc)
  ...
  let man =
    [
      `S Manpage.s_description;
      `P
        "Dissasembles an expression either from stdin, a file, or given as a \
         command-line parameter";
      `Blocks help_secs;
    ]
  in
  ( Term.(ret (const disassemble_expr $ source_file $ source_arg)),
    Term.info "dis" ~doc ~sdocs:Manpage.s_common_options ~exits ~man )
...
let cmds =
  [ parse_cmd; check_cmd; infer_cmd; eval_cmd; compile_cmd; dis_cmd; help_cmd ]
