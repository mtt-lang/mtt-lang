...
let rec subst_m term idg body =
  ...
  | Unit -> body
  | Pair (e1, e2) ->
      mkLocation (Pair (subst_m term idg e1, subst_m term idg e2)) body.loc
  ...
  | Fun (idl, t_of_id, body) ->
      mkLocation (Fun (idl, t_of_id, subst_m term idg body)) body.loc
  | App (fe, arge) ->
      mkLocation (App (subst_m term idg fe, subst_m term idg arge)) body.loc
        ...
        ( if [%equal: Id.M.t] idg i then
          ...
          | Some new_i ->
              let body_with_renamed_bound_var =
                subst_m (mkLocation (VarG new_i) body.loc) i body
              in
              Letbox
                ( new_i,
...
let rec eval_open gamma expr =
  ...
  | VarG _idg ->
      Result.fail
      @@ errorMsg "Modal variable access is not possible in a well-typed term"
           expr
  | Fun (idl, _t_of_id, body) ->
      return @@ mkLocation (Val.Clos (idl, body, gamma)) expr.loc
  | App (fe, arge) -> (
      ...
      | _ ->
          Result.fail
          @@ errorMsg "Trying to apply an argument to a non-function" fv )
