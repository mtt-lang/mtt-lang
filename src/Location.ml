...
let mkLocByPosition data start_position end_position = 
  let loc = mkPosition start_position end_position in { data; loc }

let mkLocation data location = 
  let loc = mkPosition location.start_position location.end_position in { data; loc }

let showPos pos =
  let file = 
    if (String.length pos.pos_fname) <> 0 then String.concat ["file name : "; pos.pos_fname] 
    else String.concat ["file name : "; "Not a file"] 
  in
  let line = String.concat ["line number : "; Sexp.to_string ([%sexp_of: int] pos.pos_lnum)] in
  let offs = 
