...
let rec type_pp fmt = function
  ...
  | Type.Prod { ty1; ty2 } ->
      Format.fprintf fmt "product %a %a" type_pp ty1 type_pp ty2
  | Type.Arr { dom; cod } ->
      Format.fprintf fmt "%a -> %a" type_pp dom type_pp cod
  | Type.Base { idt } -> Format.fprintf fmt "%s" idt
        ...
        [
          ...
          K
            ( "arr",
              ...
              M
                (fun ~ok ~ko -> function
                  | Type.Arr { dom; cod } -> ok dom cod
                  | _ -> ko ()) );
          K
            ( "base",
              "opaque type with name tag",
              A (API.BuiltInData.string, N),
              (* strings -> ty *)
              B (fun idt -> Type.Base { idt }),
              M
                (fun ~ok ~ko -> function
                  | Type.Base { idt } -> ok idt
                  | _ -> ko ()) );
        ];
    }
...
let get_outcome = function
  | API.Execute.Success
      ...
      let _, ty, _ =
        (API.ContextualConversion.( !< ) type_conversion).readback ~depth:0
          state term
      in
      ty
...
let linfer expr =
  let elpi, _ =
    (API.Setup.init
       ~builtins:[ Builtin.std_builtins; builtins ]
       ~basedir:"./core/elpi")
      []
  in
  let ast =
